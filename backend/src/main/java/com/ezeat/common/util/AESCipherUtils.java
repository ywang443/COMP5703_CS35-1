...
public class AESCipherUtils {
    ...
    public static String encrypt(String key, String iv, String data) {
        try {
            if (key.length() < AESCipherUtils.CIPHER_KEY_LEN) {
                int numPad = AESCipherUtils.CIPHER_KEY_LEN - key.length();

                for (int i = 0; i < numPad; i++) {
                    key += "0"; //0 pad to len 16 bytes
                }

            } else if (key.length() > AESCipherUtils.CIPHER_KEY_LEN) {
            ...
            Cipher cipher = Cipher.getInstance(AESCipherUtils.CIPHER_NAME);
